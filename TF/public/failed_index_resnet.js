...
async function getWebcamImage(webcam) {
...
async function runExample() {
    ...
    // Load the model.
    // Classify the image.
    let val = await net.predict(img, 5)
    console.log(val);
    printMatches(val);
  }
  ...
  function imagenetClassesTopK(classProbabilities, k) {
    if (!k) { k = 5; }
    const values = classProbabilities.dataSync();
  ...
  function printMatches(data) {
    ...
    for (let i of [0, 1, 2, 3, 4]) {
      results.push(`${outputClasses[i].name}: ${Math.round(outputClasses[i].probability)}%`);
    }
